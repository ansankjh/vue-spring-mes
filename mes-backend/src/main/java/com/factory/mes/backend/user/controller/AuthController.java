...
public class AuthController {
    ...
    public ResponseEntity<?> login(@RequestBody LoginRequest req, HttpSession session) {
    	...
    	// 로그인 성공 시 , 세션에 정보 저장
    	session.setAttribute("LOGIN_ID", me.getLoginId());
    	session.setAttribute("USER_SEQ", me.getUserSeq());
    	session.setAttribute("ROLE_CD", me.getRoleCd());
    ...
    public ResponseEntity<?> getUserContext(HttpSession session) { // HttpSession으로 로그인 여부를 확인하기 위해 파라미터로 받음
    	Object userSeqObj = session.getAttribute("USER_SEQ"); // 세션에서 로그인 아이디를 꺼냄(로그인 시 저장해둔 값)
    	
    	if(userSeqObj == null) { // 세션에 값이 없으면 로그인 안한 상태로 판단 
